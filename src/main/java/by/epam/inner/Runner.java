...
public class Runner {
    ...
    public static void main(String[] args) {
        ...
        try (Reader reader = new FileReader(args[0])) {
            ...
            ArrayList<Trial> trialArrayList = new ArrayList<>();
            jsonObjects.stream()
                    .map(TrialFactory::getTrial)
                    .filter(Optional::isPresent)
                    .map(Optional::get)
                    .peek(trial -> trialArrayList.add(trial))
                    .forEach(System.out::println);
            //System.out.println( trials);
            trialArrayList.forEach(System.out::println);
            //trials.forEach((trial -> System.out.println(trial.getAccount())));
